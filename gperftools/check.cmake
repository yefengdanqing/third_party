...
set(_DEP_VER 2.7)
set(_DEP_URL https://github.com/${_DEP_NAME}/${_DEP_NAME}/archive/${_DEP_NAME}-${_DEP_VER}.tar.gz)
...
if(NOT EXISTS ${_DEP_PREFIX}/lib/libtcmalloc.a)
    ...
    if(NOT EXISTS ${CMAKE_CURRENT_LIST_DIR}/build/configure)
        file(MAKE_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}/build)
        message(STATUS "Bootstrapping ${_DEP_NAME}")
        execute_process(
            COMMAND autoreconf -i -m ${CMAKE_CURRENT_LIST_DIR}/src
            WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}/build
            RESULT_VARIABLE rc)
        if(NOT "${rc}" STREQUAL "0")
            message(FATAL_ERROR "Configuring ${_DEP_NAME} - FAIL")
        endif()
        message(STATUS "Bootstrapping ${_DEP_NAME} - done")
    endif()
    ...
    if(NOT EXISTS ${CMAKE_CURRENT_LIST_DIR}/build/.libs/libtcmalloc.a)
...
if(NOT TARGET ${_DEP_NAME}::${_DEP_NAME} AND EXISTS ${_DEP_LIB_DIR}/libtcmalloc.a)
    add_library(${_DEP_NAME}::${_DEP_NAME} STATIC IMPORTED)
    if(${CMAKE_BUILD_TYPE} MATCHES "Pprof")
        set_target_properties(${_DEP_NAME}::${_DEP_NAME} PROPERTIES
            IMPORTED_LOCATION "${_DEP_LIB_DIR}/libtcmalloc_and_profiler.a"
            INTERFACE_INCLUDE_DIRECTORIES "${_DEP_INCLUDE_DIR}")
    else()
        set_target_properties(${_DEP_NAME}::${_DEP_NAME} PROPERTIES
            IMPORTED_LOCATION "${_DEP_LIB_DIR}/libtcmalloc_minimal.a"
            INTERFACE_INCLUDE_DIRECTORIES "${_DEP_INCLUDE_DIR}")
    endif()
endif()
